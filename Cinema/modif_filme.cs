...
namespace Cinema
{
    public partial class modif_filme : Form
    {
      ...
      private void datagridView_error(object sender, DataGridViewDataErrorEventArgs e)//utilizatorul nu poate lasa coloanele care nu aceept nul, vide.
      {
        ...
        private void delete_film_Click(object sender, EventArgs e)//stergere inregistrare din baza de date. in bd, pe tabela se afla constrangerea delete set cascade. daca sterg inregistrarea, o vreu stersa si in alte tabele in care apare; program_filme si rezervari_bilete. a se folosi cu grija
        {
            using (SqlConnection connection = new SqlConnection(this.dbConnection.connectrionString()))
            {
                try
